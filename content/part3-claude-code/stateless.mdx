import Terminal from '../../app/_components/Terminal'
import DiagramImage from '../../app/_components/DiagramImage'
import { Callout } from 'nextra/components'

# Stateless + /compact — ניהול ה-Context Window

## כל הודעה מעבדת את כל ההיסטוריה מחדש

Claude Code הוא stateless. בכל הודעה שאתם שולחים, המודל מקבל את כל הצ׳אט מהתחלה — כל שאלה, כל תשובה, כל קוד. ככל שהשיחה ארוכה יותר, כך יותר tokens נשרפים בכל הודעה.

**המשמעות:**
- שיחה ארוכה = יותר tokens = עלות גבוהה יותר
- כשה-context window מתמלא — Claude מתחיל "לשכוח" חלקים מוקדמים
- ביצועים יורדים ככל שההיסטוריה גדלה

## /compact — דחסו לפני שזה מתמלא

`/compact` מסכם את ההיסטוריה הארוכה לתקציר קצר, ומשחרר מקום ב-context window.

**דוגמה:**

<Terminal title="claude" lines={[
  { prompt: '>', command: '/context' },
  { output: '' },
  { output: 'Context Window Usage:' },
  { output: '  Used:      121,840 tokens (78%)' },
  { output: '  Remaining:  33,160 tokens (22%)' },
  { output: '  ⚠️  Warning: approaching limit', color: '#febc2e' },
  { output: '' },
  { prompt: '>', command: '/compact' },
  { output: '' },
  { output: 'Compressing conversation history...' },
  { output: '  Before: 156,000 tokens' },
  { output: '  After:   62,000 tokens' },
  { output: '  Freed:   94,000 tokens ✓', color: '#28c840' },
  { output: '' },
  { output: 'Summary saved. Continuing with compressed context.', color: '#28c840' },
]} />

**כלל אצבע:** אל תחכו עד שה-context מלא. עשו `/compact` כל כמה משימות — לפני שמתחילים task חדש וגדול.

## Auto Memory שורד בין sessions

בניגוד להיסטוריית הצ׳אט שנמחקת בסוף session, ה-Auto Memory של Claude Code נשמר בין sessions.

| מה | שורד? |
|----|--------|
| היסטוריית הצ׳אט | לא — מתאפסת בתחילת session חדש |
| Auto Memory (`/memory`) | כן — שמור בין sessions |
| CLAUDE.md | כן — קובץ קבוע בפרויקט |

## `/clear` — איפוס מלא

`/clear` מוחק את כל ההיסטוריה ומתחיל session נקי מאפס. בניגוד ל-`/compact` שמסכם, `/clear` באמת מאפס הכל.

**מתי להשתמש:** כשעוברים למשימה לגמרי לא קשורה. עבדתם על authentication ועכשיו עוברים ל-CSS? — `/clear`.

<Terminal title="claude" lines={[
  { prompt: '❯', command: '/clear' },
  { output: '' },
  { output: '✓ Context cleared', color: '#28c840' },
]} />

## `/clear` vs `/compact`

| | `/compact` | `/clear` |
|---|---|---|
| מה עושה | מסכם את ההיסטוריה | מוחק הכל ומתחיל מחדש |
| מתי | באמצע משימה ארוכה | בין משימות לא קשורות |
| Context | נשמר (מסוכם) | מאופס לגמרי |
| טיפ | כש-context מעל 60% | אחרי 2 תיקונים כושלים |

<DiagramImage
  src="/images/diagrams/session-lifecycle.svg"
  alt="Session Lifecycle — מתי להשתמש ב-compact, clear, continue ו-resume"
  caption="מחזור חיי הסשן: compact לאותה משימה, clear למשימה חדשה, continue/resume ליום הבא"
/>

## ניהול סשנים

Claude Code שומר את הסשנים שלכם כדי שתוכלו לחזור אליהם:

- **`continue--`** — חזרה לשיחה האחרונה. מושלם כשסגרתם טרמינל בטעות.
- **`resume--`** — בחירה מרשימת סשנים אחרונים. מציג את כולם ואתם בוחרים.
- **`/rename`** — תנו שם לסשן הנוכחי כדי למצוא אותו בקלות אחר כך.

<Terminal title="claude" lines={[
  { prompt: '$', command: 'claude --continue' },
  { output: '  ↩ Resuming last session...', color: '#cc785c' },
  { output: '' },
  { prompt: '$', command: 'claude --resume' },
  { output: '  Recent sessions:' },
  { output: '  1. auth-refactor (2 hours ago)' },
  { output: '  2. fix-css-rtl (yesterday)' },
  { output: '  3. untitled (2 days ago)' },
  { output: '' },
  { prompt: '❯', command: '/rename auth-refactor' },
  { output: '  ✓ Session renamed to "auth-refactor"', color: '#28c840' },
]} />

<Callout emoji="💡">
**התייחסו לסשנים כמו branches** — משימות שונות מקבלות סשנים נפרדים. ככה אתם לא מזהמים context של auth עם בעיות CSS, ותמיד יכולים לחזור בדיוק לאן שהפסקתם.
</Callout>

<Callout emoji="📖">
  לתיעוד הרשמי המלא: [Memory & Sessions — Claude Code Docs](https://code.claude.com/docs/en/memory)
</Callout>
